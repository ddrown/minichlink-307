...
static int InternalClearFlashOfSoftwareBreakpoint(void *dev, int i) {
    ...
    if (software_breakpoint_type[i] == 1) {
        // 32-bit instruction
        r = MCF.WriteBinaryBlob(
            dev, software_breakpoint_addy[i], 4, (uint8_t *)&previous_word_at_breakpoint_address[i]
        );
    } else {
        // 16-bit instruction
        r = MCF.WriteBinaryBlob(
            dev, software_breakpoint_addy[i], 2, (uint8_t *)&previous_word_at_breakpoint_address[i]
        );
    }
...
int RVHandleBreakpoint(void *dev, int set, uint32_t address) {
    ...
    int first_free = -1;
    for (i = 0; i < MAX_SOFTWARE_BREAKPOINTS; i++) {
        if (software_breakpoint_type[i] && software_breakpoint_addy[i] == address) break;
        if (first_free < 0 && software_breakpoint_type[i] == 0) first_free = i;
    }
