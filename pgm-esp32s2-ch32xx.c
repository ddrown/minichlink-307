...
retry:
    ...
    // printf( ">:::%d: %02x %02x %02x %02x %02x %02x\n", eps->replylen, eps->reply[0], eps->reply[1], eps->reply[2],
    // eps->reply[3], eps->reply[4], eps->reply[5] );
    if (r < 0) {
        fprintf(stderr, "Error: Got error %d when sending hid feature report.\n", r);
        return r;
    }
...
int ESPControl3v3(void *dev, int bOn) {
    ...
    if (SRemain(eps) < 2) ESPFlushLLCommands(eps);
...
int ESPReadWord(void *dev, uint32_t address_to_read, uint32_t *data) {
    struct ESP32ProgrammerStruct *eps = (struct ESP32ProgrammerStruct *)dev;
    // printf( "READ: %08x\n", address_to_read );
    if (SRemain(eps) < 6) ESPFlushLLCommands(eps);
...
int ESPWriteWord(void *dev, uint32_t address_to_write, uint32_t data) {
    ...
    if (SRemain(eps) < 10) ESPFlushLLCommands(eps);
...
static int ESPWaitForDoneOp(void *dev, int ignore) {
    struct ESP32ProgrammerStruct *eps = (struct ESP32ProgrammerStruct *)dev;
    if (SRemain(eps) < 2) ESPFlushLLCommands(dev);
    Write2LE(eps, 0x07fe);
    return 0;
}
...
retry:
    ...
    if (eps->reply[1]) {
        fprintf(
            stderr, "Error: Got code %d from ESP write algo. %d [%02x %02x %02x]\n", (char)eps->reply[1], eps->replylen,
            eps->reply[0], eps->reply[1], eps->reply[2]
        );
        if (writeretry < 10) {
            writeretry++;
            goto retry;
