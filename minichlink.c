...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strncmp(v, "-c", 2) == 0) {
            i++;
            if (i < argc) hints.serial_port = argv[i];
    ...
    int skip_startup =
        ...
        (argc > 1 && argv[1][0] == '-' && argv[1][1] == 't') | (argc > 1 && argv[1][0] == '-' && argv[1][1] == 'f') |
    ...
    keep_going:
        switch (argchar[1]) {
            ...
            case 'T': {
                ...
                if (argchar[1] == 'G') ExitGDBServer(dev);
                break;
            }
            case 's': {
                iarg += 2;
            ...
            case 'X': {
                ...
                if (MCF.VendorCommand)
                    if (MCF.VendorCommand(dev, argv[iarg++])) goto unimplemented;
                break;
            }
            case 'r': {
            ...
            case 'w': {
                ...
                // if( MCF.HaltMode ) MCF.HaltMode( dev, is_flash ? HALT_MODE_HALT_AND_RESET :
                // HALT_MODE_HALT_BUT_NO_RESET );
                if (MCF.HaltMode && is_flash) {
                    if (offset == 0x1ffff000)
                        MCF.HaltMode(dev, HALT_MODE_HALT_BUT_NO_RESET); // do not reset if writing bootloader, even if
                                                                        // it is considered flash memory
                    else
...
int DefaultSetupInterface(void *dev) {
    ...
    MCF.WriteReg32(dev, DMCFGR, 0x5aa50000 | (1 << 10)); // Bug in silicon?  If coming out of cold boot, and we don't do
                                                         // our little "song and dance" this has to be called.
...
void InternalMarkMemoryNotErased(struct InternalState *iss, uint32_t address) {
    ...
    int sector = (address & 0xffffff) / iss->sector_size;
    if (sector < MAX_FLASH_SECTORS) iss->flash_sector_status[sector] = 0;
}
...
int DefaultWriteBinaryBlob(void *dev, uint32_t address_to_write, uint32_t blob_size, uint8_t *blob) {
    ...
    if (is_flash && !iss->flash_unlocked) {
        if ((rw = InternalUnlockFlash(dev, iss))) return rw;
    }
...
static int DefaultReadWord(void *dev, uint32_t address_to_read, uint32_t *data) {
    ...
    if (iss->currentstateval == iss->ram_base + iss->ram_size) MCF.WaitForDoneOp(dev, 1); // Ignore any post-errors.
    return r;
}
...
int DefaultErase(void *dev, uint32_t address, uint32_t length, int type) {
    ...
    if (type == 1) {
        ...
        if (MCF.WaitForFlash && MCF.WaitForFlash(dev)) {
            fprintf(stderr, "Error: Wait for flash error.\n");
            return -11;
        }
...
int DefaultSetEnableBreakpoints(void *dev, int is_enabled, int single_step) {
    ...
    uint32_t DCSR;
    if (MCF.ReadCPURegister(dev, 0x7b0, &DCSR)) fprintf(stderr, "Error: DCSR could not be read\n");
    DCSR |= 0xb600;
    if (single_step)
        DCSR |= 4;
    else
    ...
    if (MCF.WriteCPURegister(dev, 0x7b0, DCSR)) fprintf(stderr, "Error: DCSR could not be read\n");
...
static int DefaultHaltMode(void *dev, int mode) {
    ...
    switch (mode) {
        ...
        case HALT_MODE_HALT_AND_RESET:
            ...
            MCF.WriteReg32(dev, DMCFGR, 0x5aa50000 | (1 << 10)); // CFGR (1<<10 == Allow output from slave)
            MCF.WriteReg32(
                dev, DMCFGR, 0x5aa50000 | (1 << 10)
            ); // Bug in silicon?  If coming out of cold boot, and we don't do
               // our little "song and dance" this has to be called.
            ...
            //		MCF.WriteReg32( dev, DMCONTROL, 0x00000001 ); // Clear Halt Request.  This is recommended, but not
            // doing it seems more stable.
        ...
        case HALT_MODE_GO_TO_BOOTLOADER:
            ...
            MCF.FlushLLCommands(dev);
            break;
        default: fprintf(stderr, "Error: Unknown halt mode %d\n", mode);
    }
...
int DefaultUnbrick(void *dev) {
    ...
    for (timeout = 0; timeout < max_timeout; timeout++) {
        ...
        MCF.WriteReg32(
            dev, DMCFGR, 0x5aa50000 | (1 << 10)
        ); // Bug in silicon?  If coming out of cold boot, and we don't do our
           // little "song and dance" this has to be called.
        MCF.WriteReg32(dev, DMCONTROL, 0x80000001); // Make the debug module work properly.
        MCF.WriteReg32(dev, DMCONTROL, 0x80000001); // Initiate a halt request.
    ...
    //  Many times we would clear the halt request, but in this case, we want to just leave it here, to prevent it from
    //  booting.
    //  TODO: Experiment and see if this is needed/wanted in cases.  NOTE: If you don't clear halt request, progarmmers
    //  can get stuck.
...
int DefaultConfigureNRSTAsGPIO(void *dev, int one_if_yes_gpio) {
    fprintf(
        stderr, "Error: DefaultConfigureNRSTAsGPIO does not work via the programmer here.  Please see the demo "
                "\"optionbytes\"\n"
    );
    return -5;
}
...
int DefaultConfigureReadProtection(void *dev, int one_if_yes_protect) {
    fprintf(
        stderr, "Error: DefaultConfigureReadProtection does not work via the programmer here.  Please see the "
                "demo \"optionbytes\"\n"
    );
    return -5;
}
...
int SetupAutomaticHighLevelFunctions(void *dev) {
    ...
    if (!MCF.SetupInterface) MCF.SetupInterface = DefaultSetupInterface;
    if (!MCF.WriteBinaryBlob) MCF.WriteBinaryBlob = DefaultWriteBinaryBlob;
    if (!MCF.ReadBinaryBlob) MCF.ReadBinaryBlob = DefaultReadBinaryBlob;
    if (!MCF.WriteWord) MCF.WriteWord = DefaultWriteWord;
    if (!MCF.WriteHalfWord) MCF.WriteHalfWord = DefaultWriteHalfWord;
    if (!MCF.WriteByte) MCF.WriteByte = DefaultWriteByte;
    if (!MCF.ReadCPURegister) MCF.ReadCPURegister = DefaultReadCPURegister;
    if (!MCF.WriteCPURegister) MCF.WriteCPURegister = DefaultWriteCPURegister;
    if (!MCF.WriteAllCPURegisters) MCF.WriteAllCPURegisters = DefaultWriteAllCPURegisters;
    if (!MCF.ReadAllCPURegisters) MCF.ReadAllCPURegisters = DefaultReadAllCPURegisters;
    if (!MCF.SetEnableBreakpoints) MCF.SetEnableBreakpoints = DefaultSetEnableBreakpoints;
    if (!MCF.ReadWord) MCF.ReadWord = DefaultReadWord;
    if (!MCF.ReadHalfWord) MCF.ReadHalfWord = DefaultReadHalfWord;
    if (!MCF.ReadByte) MCF.ReadByte = DefaultReadByte;
    if (!MCF.Erase) MCF.Erase = DefaultErase;
    if (!MCF.HaltMode) MCF.HaltMode = DefaultHaltMode;
    if (!MCF.PollTerminal) MCF.PollTerminal = DefaultPollTerminal;
    if (!MCF.WaitForFlash) MCF.WaitForFlash = DefaultWaitForFlash;
    if (!MCF.WaitForDoneOp) MCF.WaitForDoneOp = DefaultWaitForDoneOp;
    if (!MCF.PrintChipInfo) MCF.PrintChipInfo = DefaultPrintChipInfo;
    if (!MCF.Unbrick) MCF.Unbrick = DefaultUnbrick;
    if (!MCF.ConfigureNRSTAsGPIO) MCF.ConfigureNRSTAsGPIO = DefaultConfigureNRSTAsGPIO;
    if (!MCF.VoidHighLevelState) MCF.VoidHighLevelState = DefaultVoidHighLevelState;
    if (!MCF.DelayUS) MCF.DelayUS = DefaultDelayUS;
