...
int ArdulinkWriteReg32(void *dev, uint8_t reg_7_bit, uint32_t command) {
    ...
    if (serial_dev_write(&((ardulink_ctx_t *)dev)->serial, buf, 6) == -1) return -errno;

    if (serial_dev_read(&((ardulink_ctx_t *)dev)->serial, buf, 1) == -1) return -errno;
...
int ArdulinkReadReg32(void *dev, uint8_t reg_7_bit, uint32_t *commandresp) {
    ...
    if (serial_dev_write(&((ardulink_ctx_t *)dev)->serial, buf, 2) == -1) return -errno;

    if (serial_dev_read(&((ardulink_ctx_t *)dev)->serial, buf, 4) == -1) return -errno;

    *commandresp = (uint32_t)buf[0] | (uint32_t)buf[1] << 8 | (uint32_t)buf[2] << 16 | (uint32_t)buf[3] << 24;
...
int ArdulinkControl3v3(void *dev, int power_on) {
    ...
    c = power_on ? 'p' : 'P';
    if (serial_dev_write(&((ardulink_ctx_t *)dev)->serial, &c, 1) == -1) return -errno;

    if (serial_dev_read(&((ardulink_ctx_t *)dev)->serial, &c, 1) == -1) return -errno;

    if (c != '+') return -71; // EPROTO
