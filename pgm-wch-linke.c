...
static void printChipInfo(enum RiscVChip chip) {
    switch (chip) {
        case CHIP_CH32V10x: fprintf(stderr, "Detected: CH32V10x\n"); break;
        case CHIP_CH57x: fprintf(stderr, "Detected: CH57x\n"); break;
        case CHIP_CH56x: fprintf(stderr, "Detected: CH56x\n"); break;
        case CHIP_CH32V20x: fprintf(stderr, "Detected: CH32V20x\n"); break;
        case CHIP_CH32V30x: fprintf(stderr, "Detected: CH32V30x\n"); break;
        case CHIP_CH58x: fprintf(stderr, "Detected: CH58x\n"); break;
        case CHIP_CH32V003: fprintf(stderr, "Detected: CH32V003\n"); break;
    }
}

static int checkChip(enum RiscVChip chip) {
    switch (chip) {
        case CHIP_CH32V003: return 0; // Use direct mode
        case CHIP_CH32V10x:
        case CHIP_CH32V20x:
        case CHIP_CH32V30x: return 1; // Use binary blob mode
        case CHIP_CH56x:
        case CHIP_CH57x:
        case CHIP_CH58x:
        default: return -1; // Not supported yet
    }
}
...
static int LESetupInterface(void *d) {
    ...
    switch (rbuff[5]) {
        case 1: fprintf(stderr, "WCH Programmer is CH549 version %d.%d\n", rbuff[3], rbuff[4]); break;
        case 2: fprintf(stderr, "WCH Programmer is CH32V307 version %d.%d\n", rbuff[3], rbuff[4]); break;
        case 3: fprintf(stderr, "WCH Programmer is CH32V203 version %d.%d\n", rbuff[3], rbuff[4]); break;
        case 4: fprintf(stderr, "WCH Programmer is LinkB version %d.%d\n", rbuff[3], rbuff[4]); break;
        case 18: fprintf(stderr, "WCH Programmer is LinkE version %d.%d\n", rbuff[3], rbuff[4]); break;
        default: fprintf(stderr, "Unknown WCH Programmer %02x (Ver %d.%d)\n", rbuff[5], rbuff[3], rbuff[4]); break;
    }
    ...
    do {
        ...
        if (rbuff[0] == 0x81 && rbuff[1] == 0x55 && rbuff[2] == 0x01) // && rbuff[3] == 0x01 )
        {
            ...
            // Give up if too long
            if (already_tried_reset > 10) return -1;
...
static const uint8_t *GetFlashLoader(enum RiscVChip chip) {
    switch (chip) {
        case CHIP_CH32V10x: return bootloader_v1;
        case CHIP_CH32V20x:
        case CHIP_CH32V30x:
        default: return bootloader_v2;
    }
}

static int InternalLinkEHaltMode(void *d, int mode) {
    libusb_device_handle *dev = ((struct LinkEProgrammerStruct *)d)->devh;
    if (mode == ((struct LinkEProgrammerStruct *)d)->lasthaltmode) return 0;
