...
static int DecodeHexToBytes(const char *hexstr, size_t string_len, void *output, size_t out_len) {
    ...
    if (out_len < (string_len / 2) || (string_len % 2) != 0) return -1;
    uint8_t *out = (uint8_t *)output;
    for (size_t i = 0; i < string_len; i += 2) {
        int nibble1, nibble2;
        if ((nibble1 = fromhex(hexstr[i])) < 0) return nibble1; // error
        if ((nibble2 = fromhex(hexstr[i + 1])) < 0) return nibble2; // error
...
static int ReadHex(char **instr, int numhex, uint32_t *outwrite) {
    ...
    *outwrite = 0;
    ...
    int scanhex = numhex;
    if (scanhex < 0) scanhex = strlen(str);
...
void HandleGDBPacket(void *dev, char *data, int len) {
    ...
    switch (cmd) {
            ...
            else if (StringMatch(data, "Rcmd,")) // "monitor <command>"
            {
                ...
                if (strlen(cmdHex) > 1) {
                    ...
                    if (StringMatch(cmd, "halt")) {
                        ...
                        static const char helptext[] = "minichlink GDB monitor help:\n"
                                                       "- halt: Halt execution\n"
        ...
        case 'm': {
            ...
            if (RVReadMem(dev, address_to_read, pl, length_to_read) < 0) goto err;
        ...
        switch (gdbbufferstate) {
            ...
            case 3: {
                ...
                if (gdbrunningcsum == gdbchecksum) {
                    ...
                    printf(
                        "Checksum Error: Got %02x expected %02x / len: %d\n", gdbrunningcsum, gdbchecksum,
                        gdbbufferplace
                    );
                    int i;
