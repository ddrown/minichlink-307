...
WINSETUPAPI BOOL SetupDiEnumDeviceInterfaces(
	HDEVINFO DeviceInfoSet, PSP_DEVINFO_DATA DeviceInfoData, const GUID *InterfaceClassGuid, DWORD MemberIndex,
...
WINSETUPAPI BOOL SetupDiEnumDeviceInfo(HDEVINFO DeviceInfoSet, DWORD MemberIndex, PSP_DEVINFO_DATA DeviceInfoData);
...
WINSETUPAPI BOOL SetupDiDestroyDeviceInfoList(HDEVINFO DeviceInfoSet);
#else
#include <setupapi.h>
#include <winioctl.h>
#endif
...
static HANDLE open_device(const char *path, BOOL enumerate) {
	...
	handle = CreateFileA(
		path, desired_access, share_mode, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, /*FILE_ATTRIBUTE_NORMAL,*/
		0
	);
...
struct hid_device_info HID_API_EXPORT *HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
	...
	for (;;) {
		...
		res = SetupDiEnumDeviceInterfaces(
			device_info_set, NULL, &InterfaceClassGuid, device_index, &device_interface_data
		);
		...
		res = SetupDiGetDeviceInterfaceDetailA(
			device_info_set, &device_interface_data, device_interface_detail_data, required_size, NULL, NULL
		);
		...
		for (i = 0;; i++) {
			...
			res = SetupDiGetDeviceRegistryPropertyA(
				device_info_set, &devinfo_data, SPDRP_CLASS, NULL, (PBYTE)driver_name, sizeof(driver_name), NULL
			);
			if (!res) goto cont;
			...
			if (strcmp(driver_name, "HIDClass") == 0) {
				...
				res = SetupDiGetDeviceRegistryPropertyA(
					device_info_set, &devinfo_data, SPDRP_DRIVER, NULL, (PBYTE)driver_name, sizeof(driver_name), NULL
				);
				if (res) break;
			}
		}
...
int HID_API_EXPORT HID_API_CALL hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length) {
	...
	res = DeviceIoControl(
		dev->device_handle, IOCTL_HID_GET_FEATURE, data, (DWORD)length, data, (DWORD)length, &bytes_returned, &ol
	);
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_manufacturer_string(hid_device *dev, wchar_t *string, size_t maxlen) {
	...
	res = HidD_GetManufacturerString(
		dev->device_handle, string, (ULONG)(sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS))
	);
	if (!res) {
...
hid_get_indexed_string(hid_device *dev, int string_index, wchar_t *string, size_t maxlen) {
	...
	res = HidD_GetIndexedString(
		dev->device_handle, string_index, string, (ULONG)(sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS))
	);
	if (!res) {
...
static int get_string_property(IOHIDDeviceRef device, CFStringRef prop, wchar_t *buf, size_t len) {
	...
	if (str) {
		...
		chars_copied = CFStringGetBytes(
			str, range, kCFStringEncodingUTF32LE, (char)'?', FALSE, (UInt8 *)buf, len * sizeof(wchar_t), &used_buf_len
		);
...
static io_service_t hidapi_IOHIDDeviceGetService(IOHIDDeviceRef device) {
	...
	if (iokit_framework == NULL) {
		...
		if (iokit_framework != NULL) dynamic_IOHIDDeviceGetService = dlsym(iokit_framework, "IOHIDDeviceGetService");
	}
...
/* The Run Loop calls this function for each input report received.
   ...
   hid_read(). */
static void hid_report_callback(
	void *context, IOReturn result, void *sender, IOHIDReportType report_type, uint32_t report_id, uint8_t *report,
 ...
 * Example: "IOService:/AppleACPIPlatformExpert/PCI0@0/AppleACPIPCI/EHC1@1D,7/AppleUSBEHCI/PLAYSTATION(R)3
...
hid_device *HID_API_EXPORT hid_open_path(const char *path) {
	...
	if (ret == kIOReturnSuccess) {
		...
		IOHIDDeviceRegisterInputReportCallback(
			dev->device_handle, dev->input_report_buf, dev->max_input_report_len, &hid_report_callback, dev
		);
...
static int set_report(hid_device *dev, IOHIDReportType type, const unsigned char *data, size_t length) {
	...
	if (!dev->disconnected) {
		res = IOHIDDeviceSetReport(
			dev->device_handle, type, data[0], /* Report ID*/
...
static int cond_timedwait(
	const hid_device *dev, pthread_cond_t *cond, pthread_mutex_t *mutex, const struct timespec *abstime
) {
	while (!dev->input_reports) {
...
int HID_API_EXPORT hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length) {
	...
	res = IOHIDDeviceGetReport(
		dev->device_handle, kIOHIDReportTypeFeature, data[0], /* Report ID */
		data, &len
	);
	if (res == kIOReturnSuccess)
		return len;
...
void HID_API_EXPORT hid_close(hid_device *dev) {
	...
	if (!dev->disconnected) {
		IOHIDDeviceRegisterInputReportCallback(
			dev->device_handle, dev->input_report_buf, dev->max_input_report_len, NULL, dev
		);
 ...
 */
static int parse_uevent_info(
	const char *uevent, int *bus_type, unsigned short *vendor_id, unsigned short *product_id, char **serial_number_utf8,
	...
	if (udev_dev) {
		...
		if (hid_dev) {
			...
			ret = parse_uevent_info(
				udev_device_get_sysattr_value(hid_dev, "uevent"), &bus_type, &dev_vid, &dev_pid, &serial_number_utf8,
				&product_name_utf8
			);
...
hid_device *HID_API_EXPORT hid_open_path(const char *path) {
	...
	if (dev->device_handle > 0) {
		...
		if (res < 0 && !g_hidapiSuppress) perror("HIDIOCGRDESCSIZE");
...
int HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds) {
	...
	if (milliseconds >= 0) {
		...
		if (ret == -1 || ret == 0) {
			...
			if (fds.revents & (POLLERR | POLLHUP | POLLNVAL)) return -1;
		}
	}

	bytes_read = read(dev->device_handle, data, length);
	if (bytes_read < 0 && (errno == EAGAIN || errno == EINPROGRESS)) bytes_read = 0;
...
int HID_API_EXPORT hid_send_feature_report(hid_device *dev, const unsigned char *data, size_t length) {
	...
	if (res < 0 && !g_hidapiSuppress) perror("ioctl (SFEATURE)");
...
int HID_API_EXPORT hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length) {
	...
	if (res < 0 && !g_hidapiSuppress) perror("ioctl (GFEATURE)");
...
static uint32_t get_bytes(uint8_t *rpt, size_t len, size_t num_bytes, size_t cur) {
	...
	if (num_bytes == 0)
		...
		return (
			rpt[cur + 4] * 0x01000000 + rpt[cur + 3] * 0x00010000 + rpt[cur + 2] * 0x00000100 +
...
static int get_usage(uint8_t *report_descriptor, size_t size, unsigned short *usage_page, unsigned short *usage) {
	...
	while (i < size) {
		...
		if (usage_page_found && usage_found) return 0; /* success */
 ...
 */
static inline int libusb_get_string_descriptor(
	libusb_device_handle *dev, uint8_t descriptor_index, uint16_t lang_id, unsigned char *data, int length
) {
	return libusb_control_transfer(
		dev, LIBUSB_ENDPOINT_IN | 0x0, /* Endpoint 0 IN */
		LIBUSB_REQUEST_GET_DESCRIPTOR, (LIBUSB_DT_STRING << 8) | descriptor_index, lang_id, data, (uint16_t)length, 1000
	);
}
...
static wchar_t *get_usb_string(libusb_device_handle *dev, uint8_t idx) {
	...
	if (!is_language_supported(dev, lang)) lang = get_first_language(dev);
	...
	if (outbytes >= sizeof(wbuf[0])) *((wchar_t *)outptr) = 0x00000000;
...
static char *make_path(libusb_device *dev, int interface_number) {
	char str[64];
	snprintf(
		str, sizeof(str), "%04x:%04x:%02x", libusb_get_bus_number(dev), libusb_get_device_address(dev), interface_number
	);
	str[sizeof(str) - 1] = '\0';
...
int HID_API_EXPORT hid_init(void) {
	if (!usb_context) {
		...
		/* Init Libusb */
		if (libusb_init(&usb_context)) return -1;
...
struct hid_device_info HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
	...
	while ((dev = devs[i++]) != NULL) {
		...
		if (res < 0) libusb_get_config_descriptor(dev, 0, &conf_desc);
		if (conf_desc) {
					...
					if (intf_desc->bInterfaceClass == LIBUSB_CLASS_HID) {
						...
						if ((vendor_id == 0x0 || vendor_id == dev_vid) &&
							...
							if (res >= 0) {
								...
								if (desc.iProduct > 0) cur_dev->product_string = get_usb_string(handle, desc.iProduct);
...
#ifdef INVASIVE_GET_USAGE
								{
									...
									if (res >= 0) {
										...
										if (res >= 0) {
											...
											LOG("libusb_control_transfer() for getting the HID report failed with "
											    "%d\n",
											    res);

										/* Release the interface */
										res = libusb_release_interface(handle, interface_num);
										if (res < 0) LOG("Can't release the interface.\n");
									} else
									...
									if (detached) {
										...
										if (res < 0) LOG("Couldn't re-attach kernel driver.\n");
									}
#endif
								}
#endif /* INVASIVE_GET_USAGE */
...
static void *read_thread(void *param) {
	...
	libusb_fill_interrupt_transfer(
		dev->transfer, dev->device_handle, dev->input_endpoint, buf, length, read_callback, dev, 5000 /*timeout*/
	);
	...
	while (!dev->cancelled) libusb_handle_events_completed(usb_context, &dev->cancelled);
...
hid_device *HID_API_EXPORT hid_open_path(const char *path) {
	...
	while ((usb_dev = devs[d++]) != NULL) {
		...
		if (libusb_get_active_config_descriptor(usb_dev, &conf_desc) < 0) continue;
		for (j = 0; j < conf_desc->bNumInterfaces; j++) {
			const struct libusb_interface *intf = &conf_desc->interface[j];
			for (k = 0; k < intf->num_altsetting; k++) {
				const struct libusb_interface_descriptor *intf_desc;
...
int HID_API_EXPORT hid_write(hid_device *dev, const unsigned char *data, size_t length) {
	...
	if (dev->output_endpoint <= 0) {
		...
		res = libusb_control_transfer(
			dev->device_handle, LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE | LIBUSB_ENDPOINT_OUT,
			0x09 /*HID Set_Report*/, (2 /*HID output*/ << 8) | report_number, dev->interface, (unsigned char *)data,
			length, 1000 /*timeout millis*/
		);
		...
		res = libusb_interrupt_transfer(
			dev->device_handle, dev->output_endpoint, (unsigned char *)data, length, &actual_length, 1000
		);
...
int HID_API_EXPORT hid_send_feature_report(hid_device *dev, const unsigned char *data, size_t length) {
	...
	res = libusb_control_transfer(
		dev->device_handle, LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE | LIBUSB_ENDPOINT_OUT,
		0x09 /*HID set_report*/, (3 /*HID feature*/ << 8) | report_number, dev->interface, (unsigned char *)data,
		length, 1000 /*timeout millis*/
	);
...
int HID_API_EXPORT hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length) {
	...
	res = libusb_control_transfer(
		dev->device_handle, LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE | LIBUSB_ENDPOINT_IN,
		0x01 /*HID get_report*/, (3 /*HID feature*/ << 8) | report_number, dev->interface, (unsigned char *)data,
		length, 1000 /*timeout millis*/
	);

	if (res < 0) return -1;

	if (skipped_report_id) res++;
